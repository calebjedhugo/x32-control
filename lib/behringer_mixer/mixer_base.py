...
class MixerBase:
    ...
    def __init__(self, **kwargs):
        ...
        self._pending_queries = {}    # address -> asyncio.Event
        self._matched_responses = {}  # address -> data tuple
    ...
    def msg_handler(self, addr, *data):
        ...
        if addr == "/*":
            ...
            updates = []
        # Address-matched response delivery
        if addr in self._pending_queries:
            self._matched_responses[addr] = data[:]
            self._pending_queries[addr].set()
    ...
    async def query(self, address):
        """Send and wait for the response matching this specific address.

        Uses per-address event matching to avoid race conditions when
        multiple queries are in flight. Bypasses send() to avoid clearing
        the global buffer and adding unnecessary delay.
        """
        event = asyncio.Event()
        self._pending_queries[address] = event
        self._matched_responses.pop(address, None)
        self.server.send_message(address, None)
        try:
            await asyncio.wait_for(event.wait(), timeout=0.5)
            return self._matched_responses.get(address)
        except asyncio.TimeoutError:
            return None
        finally:
            self._pending_queries.pop(address, None)
            self._matched_responses.pop(address, None)
