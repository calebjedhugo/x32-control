...
- **Jun 10, 2026**: Auto-awesome hardening — moved deterministic glue from LLM workers into scripts. `control.py --batch` now validates before applying (address allowlist, value/index ranges, mute/scene/routing/DCA refused, drastic-move backstop vs `old_value`), writes the changelog itself (`--changelog/--phase/--iter`), exits 0/1/2/3 (all/load/validation/partial), keeps failures in `<batch>.failed.json`, prints `BATCH_RESULT {...}`. New `prepare_pass.py` generates the worker context brief (classification + channel lists + VENUE.md gain targets). `classify_channel()` now reads personnel names from CHANNELS.md and returns `unknown` (not `vocal`) for unrecognized labels. `splice_rta.py` keeps its source by default (`--delete-source` for old behavior), exits 1 if nothing spliced. `extract.py` errors on un-analyzed captures and invalid channel numbers. `update_peaks.py` rejects out-of-range channels/offsets. `meter_collector.py` output includes `ended_by` (stop_file vs max_duration).
