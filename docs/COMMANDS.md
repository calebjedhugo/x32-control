...
```
Executes all changes in one mixer connection. File format: `[{"address": "/ch/01/eq/2/g", "value": 0.55, "old_value": 0.5, "human": "+1.5 dB (was 0 dB)", "ch": "ch01", "label": "Tammy", "reason": "..."}, ...]` (only `address`/`value` required). Validates everything before applying: address allowlist, 0.0-1.0 value ranges, integer indices (compressor ratio 0-11, knee 0-5), and refuses mute/scene/routing/DCA changes. Then **pre-write verifies each command against the live board**: a claimed `old_value` that disagrees with the live read (tolerance 0.03) is refused, and move size is checked against the live value (>0.34 raw refused) — a worker's old_value claim is never load-bearing. With `--changelog`, appends one JSONL line per applied change. Deletes the batch file on completion; failed sends go to `<file>.failed.json`; refusals are listed in `BATCH_RESULT.refused_detail` (never retry those — re-analyze instead). Exit codes: 0 = all applied, 1 = load error, 2 = validation failed (batch kept), 3 = partial (failures or refusals). Last stdout line is machine-readable: `BATCH_RESULT {...}`.
